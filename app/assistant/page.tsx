...
function AssistantPageContent() {
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(180deg, #0D1B0F 0%, #1A3320 40%, #0D2010 100%)',
      ...
      <div style={{
        ...
        backgroundImage: 'radial-gradient(ellipse at 20% 50%, rgba(46,204,113,0.08) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(39,174,96,0.06) 0%, transparent 50%)',
      ...
      <div style={{
        ...
        <button onClick={() => router.push(`/?lang=${language}`)} style={{
        ...
        <button onClick={() => setLanguage(l => l === 'EN' ? 'TA' : 'EN')} style={{
          background: 'rgba(46,204,113,0.15)',
          border: '1px solid rgba(46,204,113,0.4)',
          borderRadius: '20px', padding: '5px 14px',
          fontSize: '12px', cursor: 'pointer',
          fontWeight: 700, color: '#2ECC71',
          letterSpacing: '0.5px'
        }}>{language === 'EN' ? 'TA' : 'EN'}</button>
      </div>

      {/* MESSAGES */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', paddingBottom: '120px', zIndex: 1 }}>
        {messages.map((msg) => {
          ...
          if (msg.isLoading) return (
            <div key={msg.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '12px' }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#1a1a1a', marginBottom: '4px' }}>{t.analyzing}</div>
          ...
          /* FORM BUBBLE */
          if (msg.isForm && msg.questions) return (
            <div key={msg.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '12px' }}>
              <div style={{
                width: '34px', height: '34px', borderRadius: '50%',
                background: 'linear-gradient(135deg, #2ECC71, #27AE60)',
              ...
              <div style={{
                background: 'rgba(255,255,255,0.97)',
                borderRadius: '4px 18px 18px 18px',
                padding: '16px', maxWidth: '85%',
                boxShadow: '0 8px 32px rgba(0,0,0,0.4)'
              }}>
                <p style={{ fontWeight: 700, marginBottom: '14px', fontSize: '14px', color: '#1a1a1a' }}>{t.moreDetails}</p>
                {msg.questions.map(q => (
                ...
                <button onClick={() => handleFormSubmit(msg.questions!)} style={{
                  background: 'linear-gradient(135deg, #2ECC71, #27AE60)',
          ...
          if (msg.isError) return (
            <div key={msg.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '12px' }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: '13px', color: '#DC2626', marginBottom: '10px' }}>{msg.text}</p>
                <button onClick={() => runQuery(initialQuery, context)} style={{
              ...
              <div key={msg.id} style={{ marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '10px' }}>
                  ...
                  <div style={{
                    background: 'rgba(255,255,255,0.97)',
                    borderRadius: '4px 18px 18px 18px',
                    padding: '14px 16px', maxWidth: '82%',
                    boxShadow: '0 8px 32px rgba(0,0,0,0.4)'
                  }}>
                    {/* Confidence */}
                    <span style={{
                      display: 'inline-block', marginBottom: '8px',
                      background: msg.result.confidence_level === 'high' ? '#DCFCE7' :
                                  msg.result.confidence_level === 'medium' ? '#FEF9C3' : '#FEE2E2',
                      color: msg.result.confidence_level === 'high' ? '#16A34A' :
                             msg.result.confidence_level === 'medium' ? '#CA8A04' : '#DC2626',
                    ...
                    {/* AUDIO PLAYER - always show for TA */}
                    {(msg.result.language === 'TA' || language === 'TA') && (
                      <div style={{
                        ...
                        <button onClick={toggleAudio} style={{
                          ...
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          boxShadow: '0 2px 8px rgba(46,204,113,0.4)', flexShrink: 0
                        }}>
                          {playing ? '⏸' : '▶'}
                        </button>
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', gap: '2px', marginTop: '4px', alignItems: 'flex-end', height: '16px' }}>
                            {[3,5,4,6,3,5,4,3,5,6,4,3].map((h, i) => (
                              <div key={i} style={{
                                width: '3px', height: `${h * (playing ? (Math.random() > 0.5 ? 1.4 : 0.8) : 1)}px`,
                                background: '#2ECC71', borderRadius: '2px', opacity: audioUrl ? 1 : 0.4
                              }} />
                            ))}
                          </div>
                        </div>
                    ...
                    return (
                      <div key={i} style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '12px', fontWeight: 700, color: '#1a1a1a', marginBottom: '5px', lineHeight: 1.3 }}>
                            {insight.title}
                          </div>
                        ...
                        <div style={{ fontSize: '10px', color: style.border, fontWeight: 600, marginTop: '6px' }}>
                          ↗ {t.viewRec}
                        </div>
                      </div>
                    );
                ...
                {/* QUICK ACTIONS */}
                <div style={{ marginLeft: '44px', marginTop: '10px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {['What are the risks?', 'Best time to sell?', 'Alternative crops?'].map(q => (
                    <button key={q} onClick={() => runQuery(q, context)} style={{
      ...
      <div style={{
        ...
        <button onClick={handleVoice} style={{
          ...
          background: listening ? 'rgba(231,76,60,0.3)' : 'rgba(255,255,255,0.1)',
          border: `1px solid ${listening ? 'rgba(231,76,60,0.6)' : 'rgba(255,255,255,0.2)'}`,
          cursor: 'pointer', fontSize: '18px',
          display: 'flex', alignItems: 'center', justifyContent: 'center'
        }}>🎤</button>
        <button onClick={handleSend} disabled={!input.trim()} style={{
          width: '42px', height: '42px', borderRadius: '50%',
          background: input.trim() ? 'linear-gradient(135deg, #2ECC71, #27AE60)' : 'rgba(255,255,255,0.15)',
          border: 'none', cursor: input.trim() ? 'pointer' : 'default',
          fontSize: '18px', color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: input.trim() ? '0 4px 12px rgba(46,204,113,0.4)' : 'none',
          transition: 'all 0.2s'
        }}>➤</button>
      </div>
  ...
  return (
    <Suspense fallback={
      <div style={{
        ...
        <div style={{ marginTop: '16px', fontSize: '16px', color: 'rgba(255,255,255,0.7)',
